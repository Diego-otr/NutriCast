...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.trim() || !password.trim() || !groupName.trim()) {
      ...
      <div className="w-full max-w-md px-4 py-12 flex-1 flex flex-col justify-center">
        <div className="bg-[#f7faeb] border border-[#1b3d30] rounded-3xl p-6 sm:p-8 shadow-xl flex flex-col gap-6">
          <div className="flex flex-col gap-1 text-center">
            <h2 className="text-2xl font-extrabold text-[#1b3d30]">Crear Cuenta</h2>
            <p className="text-xs text-zinc-600">Regístrate para comenzar a monitorear tu nutrición y crear tu grupo.</p>
          </div>
          ...
          <div className="text-center text-xs text-zinc-600">
            ...
            <Link href="/login" className="font-bold text-[#368482] hover:underline">
              Inicia sesión aquí
            </Link>
          </div>
        </div>
